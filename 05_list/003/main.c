...
void voice_history_read_from_file(VOICE_HISTORY_NODE ** list_new_voice_p, VOICE_HISTORY_NODE ** list_ctrl_p, char *path);
...
int main(void)
{
	...
	print_list(head);
//	voice_history_read_from_file(&list_new_voice_p, &list_ctrl_p, "./voice_history.txt");
	voice_history_read_from_file(&head, &list_ctrl_p, "./voice_history.txt");
	...
	voice_history_write_to_file(list_new_voice_p->prev, "./voice_history.txt");
	destory_list(&head);
	return 0;
}
...
void voice_history_read_from_file(VOICE_HISTORY_NODE ** list_new_voice_p, VOICE_HISTORY_NODE ** list_ctrl_p, char *path)
{
	...
	while(0 < fscanf(fp, "%s\n", buf))
	{
		voice_history_play_new_voice(list_new_voice_p, list_ctrl_p, buf);
		memset(buf, 0, 1024);
	}
