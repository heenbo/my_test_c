	...
	while(0 < fscanf(fp, "%[^\n]\n", input_buf))
	{
		...
		while((point_key = strsep(&input_str, " \t,\r\n")))
		{
			...
			else//如果是类标签，则写入label_column_str
			{
				...
				if(('\n' == label_column_str[strlen(label_column_str)]) || ('\r' == label_column_str[strlen(label_column_str)]))
				{
					label_column_str[strlen(label_column_str)] = '\0';
				}
			}
			i++;
			//			printf("value = %s\n", point_key);
...
//	printf("change_file_name: dir_path:%s\n", dir_path);
	...
	printf("输入文件在当前目录的 input_file 目录下\n");
	printf("输出文件在当前目录的 output_file 目录下\n");
	printf("数据维度可能受 \\r 的影响而不对，明天再看一下, 非法情况分析下次弄，不难的\n\n\n");
	return 0;
}
